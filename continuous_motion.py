...
def generate_move_script(pickup_point, master1_point, master2_point, num_layers, num_pallets, layer_config, transfer_point,
    ...
    for layer in range(num_layers):
        ...
        for i in range(max_boxes):
            if not initial_pre_pickup_move_done:
                ...
                script += f"    movel({pre_pickup}, a=1.0, v=1.5, t=0, r=0)\n"
            ...
            if i < len(box_coords_pallet1):
                ...
                script += f"    movel({pickup_point}, a=1, v=0.5, t=0, r=0)\n"
                script += f"    movel({pre_pickup}, a=2.8, v=2.8, t=0, r=0.15)\n"

                if transfer_point:
                    script += f"    movel({transfer_point}, a=2.8, v=2.8, t=0, r=0.15)\n"

                pre_place_rotated = apply_rotation(pre_place.copy(), rotation_angle)
                box_abs_rotated = apply_rotation(box_abs.copy(), rotation_angle)

                script += f"    movel({pre_place_rotated}, a=2.8, v=2.8, t=0, r=0.15)\n"
                script += f"    movel({box_abs_rotated}, a=1, v=0.5, t=0, r=0)\n"
                script += f"    movel({pre_place_rotated}, a=2.8, v=2.8, t=0, r=0.15)\n"
    ...
    safe_position[2] += 0.1
    script += f"    movel({safe_position}, a=0.5, v=0.1, t=0, r=0)\n"
...
if __name__ == "__main__":
    ...
    try:
        ...
        move_script = generate_move_script(pickup_point, master1_point, master2_point, num_layers, num_pallets, layer_config, transfer_point,
                                           box_coords_odd_pallet1, box_coords_even_pallet1, box_coords_odd_pallet2, box_coords_even_pallet2)
